...
import type { EventHandler, ClickEvent, DragEvent, MouseEvent } from './CanvasEngine';
...
export class HeatmapEventHandler implements EventHandler {
  ...
  onClick(event: ClickEvent): void {
    ...
    // 标记已处理，阻止 onDragStart 重复调用
    this.clickProcessed = true;
    setTimeout(() => { this.clickProcessed = false; }, 100);
