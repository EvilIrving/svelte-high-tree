 ...
 */
export function computeVisibleNodes(flatNodes, expandedSet) {
    const visible = [];
    let i = 0;
 ...
 * 只显示匹配节点 + 其祖先路径
 */
export function computeFilteredVisibleNodes(flatNodes, expandedSet, filterSet) {
    // 如果没有过滤条件，走正常逻辑
    if (filterSet.size === 0) {
 ...
 * 切换节点展开状态
 */
export function toggleExpand(nodeId, expandedSet) {
 ...
 * 展开到指定节点（展开所有祖先）
 */
export function expandToNode(nodeId, expandedSet, nodeMap) {
    const newSet = new Set(expandedSet);
 ...
 * 利用 subtreeEnd 批量操作
 */
export function toggleSubtree(nodeId, expand, flatNodes, expandedSet, nodeMap) {
    const newSet = new Set(expandedSet);
    const node = nodeMap.get(nodeId);
    if (!node)
        return newSet;
 ...
 * 展开多个节点（用于搜索结果定位）
 */
export function expandMultiple(nodeIds, expandedSet) {
    return new Set([...expandedSet, ...nodeIds]);
}
/**
 * 收起同级其他节点（手风琴模式）
 */
export function collapseSiblings(nodeId, flatNodes, expandedSet, nodeMap) {
    const newSet = new Set(expandedSet);
    const node = nodeMap.get(nodeId);
    if (!node || node.parentId === null)
        return newSet; // 根节点无同级
    // 找到父节点的所有子节点
